...
function send_wk_list() {
    ...
    PMM_LIST=`sed -n '3p' $1/$SDK_INFO`
    DAY=`date +%-d`
    MONTH=`date +%-m`
    YEAR=`date +%-Y`
    STRING_M_TITLE="month work list"
    STRING_Y_TITLE="year work list"
    #debug_wk_list=true
    ...
    if [ "$DAY" == "25" -o "$debug_wk_list" == "true" ]; then
        HOUR=`date +%-H`
        
        if [ "$HOUR" == "11" -o "$debug_wk_list" == "true" ]; then
        ...
        if [ "$HOUR" == "12" -o "$debug_wk_list" == "true" ]; then
            if [ -e $MK_WK_FLAG_FILE ]; then
                ##############################Send one month worklist################
                ./mkwklist.sh
                rm "work_list_from*.csv"
                WK_LIST_FILE=`ls *.csv`
                ...
                echo "Subject: $SDK_NAME $MONTH $STRING_M_TITLE" >> $MAIL_WK_LIST
                ...
                #######################End of send one month worklist##############

                ######################Send one year worklist#######################
                if [ "$MONTH" == "12" -o "$debug_wk_list" == "true" ]; then
                    LAST_YEAR=$YEAR
                    let LAST_YEAR-=1
                    ./mkwklist.sh $LAST_YEAR-12-25
                    WK_LIST_FILE=`ls *.csv`
                    echo "From: MidGitLog<MidGitLog@cvte.cn>" > $MAIL_WK_LIST
                    echo "Content-type: text/html;charset=UTF-8" >> $MAIL_WK_LIST
                    echo "To: $PMM_LIST" >> $MAIL_WK_LIST
                    echo "Subject: $SDK_NAME $YEAR $STRING_Y_TITLE" >> $MAIL_WK_LIST
                    echo "<pre>" >> $MAIL_WK_LIST
                    echo "<table border=1 cellSpacing=0 cellPadding=5>" >> $MAIL_WK_LIST
                    while read LINE
                    do
